...
def get_color_based_on_parameter(row, parameter):
    ...
    # Default to 'blue' if parameter is not found in the colormap
    color = colormap.get(parameter, 'red')
    return color
...
def get_color_based_on_time(row):
    ...
    datetime_value = pd.to_datetime(
        row['DateTime'], format='%Y-%m-%d %H:%M:%S')
...
def app():
    ...
    selected_parameter = st.sidebar.radio(
        "Select parameter for visualization", ['Markers', 'Heatmap'])
    ...
    UP_AREA = [(min_latitude + max_latitude) / 2,
    ...
    if selected_parameter == 'Markers':
        selected_mapping_parameter = st.sidebar.radio(
            "Select parameter for color mapping", [None, 'DateTime'])
